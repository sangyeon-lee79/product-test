...
VALUES
  ('mc-metric', 'metric', 210, 'active', datetime('now'), datetime('now')),
  ('mc-unit',   'unit',   211, 'active', datetime('now'), datetime('now'));

-- ─── metric 아이템 ──────────────────────────────────────────────────────────
INSERT OR IGNORE INTO master_items (id, category_id, parent_item_id, code, sort_order, status, metadata, created_at, updated_at)
VALUES
  ('mi-metric-blood-glucose', (SELECT id FROM master_categories WHERE code='metric'), NULL, 'blood_glucose', 1, 'active', '{}', datetime('now'), datetime('now')),
  ('mi-metric-insulin-dose',  (SELECT id FROM master_categories WHERE code='metric'), NULL, 'insulin_dose',  2, 'active', '{}', datetime('now'), datetime('now')),
  ('mi-metric-meal-amount',   (SELECT id FROM master_categories WHERE code='metric'), NULL, 'meal_amount',   3, 'active', '{}', datetime('now'), datetime('now')),
  ('mi-metric-food-weight',   (SELECT id FROM master_categories WHERE code='metric'), NULL, 'food_weight',   4, 'active', '{}', datetime('now'), datetime('now')),
  ('mi-metric-water-intake',  (SELECT id FROM master_categories WHERE code='metric'), NULL, 'water_intake',  5, 'active', '{}', datetime('now'), datetime('now')),
  ('mi-metric-duration',      (SELECT id FROM master_categories WHERE code='metric'), NULL, 'duration',      6, 'active', '{}', datetime('now'), datetime('now')),
  ('mi-metric-body-weight',   (SELECT id FROM master_categories WHERE code='metric'), NULL, 'body_weight',   7, 'active', '{}', datetime('now'), datetime('now'));

-- ─── unit 아이템 ────────────────────────────────────────────────────────────
INSERT OR IGNORE INTO master_items (id, category_id, parent_item_id, code, sort_order, status, metadata, created_at, updated_at)
VALUES
  ('mi-unit-mg-dl',  (SELECT id FROM master_categories WHERE code='unit'), NULL, 'mg_dl',  1, 'active', '{"symbol":"mg/dL"}', datetime('now'), datetime('now')),
  ('mi-unit-mmol-l', (SELECT id FROM master_categories WHERE code='unit'), NULL, 'mmol_l', 2, 'active', '{"symbol":"mmol/L"}', datetime('now'), datetime('now')),
  ('mi-unit-iu',     (SELECT id FROM master_categories WHERE code='unit'), NULL, 'iu',     3, 'active', '{"symbol":"IU"}', datetime('now'), datetime('now')),
  ('mi-unit-g',      (SELECT id FROM master_categories WHERE code='unit'), NULL, 'g',      4, 'active', '{"symbol":"g"}', datetime('now'), datetime('now')),
  ('mi-unit-kg',     (SELECT id FROM master_categories WHERE code='unit'), NULL, 'kg',     5, 'active', '{"symbol":"kg"}', datetime('now'), datetime('now')),
  ('mi-unit-ml',     (SELECT id FROM master_categories WHERE code='unit'), NULL, 'ml',     6, 'active', '{"symbol":"mL"}', datetime('now'), datetime('now')),
  ('mi-unit-kcal',   (SELECT id FROM master_categories WHERE code='unit'), NULL, 'kcal',   7, 'active', '{"symbol":"kcal"}', datetime('now'), datetime('now')),
  ('mi-unit-min',    (SELECT id FROM master_categories WHERE code='unit'), NULL, 'min',    8, 'active', '{"symbol":"min"}', datetime('now'), datetime('now'));
