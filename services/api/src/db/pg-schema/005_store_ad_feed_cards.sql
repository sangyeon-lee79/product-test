...
(gen_random_uuid()::text, 'admin.feed_card.rotation_mode', 'admin', true, '표시 순서', 'Display Order', '表示順', '显示顺序', '顯示順序', 'Orden de visualización', 'Ordre d''affichage', 'Anzeigereihenfolge', 'Ordem de exibição', 'Thứ tự hiển thị', 'ลำดับแสดง', 'Urutan tampilan', 'ترتيب العرض', now(), now()),
(gen_random_uuid()::text, 'admin.feed_card.rotation_sequential', 'admin', true, '1위부터 순서대로', 'Sequential from 1st', '1位から順に', '从第1名开始', '從第1名開始', 'Secuencial desde 1°', 'Séquentiel depuis 1er', 'Sequentiell ab 1.', 'Sequencial do 1°', 'Tuần tự từ #1', 'เรียงจากอันดับ 1', 'Berurutan dari #1', 'تسلسلي من الأول', now(), now()),
(gen_random_uuid()::text, 'admin.feed_card.rotation_random', 'admin', true, '랜덤', 'Random', 'ランダム', '随机', '隨機', 'Aleatorio', 'Aléatoire', 'Zufällig', 'Aleatório', 'Ngẫu nhiên', 'สุ่ม', 'Acak', 'عشوائي', now(), now()),
(gen_random_uuid()::text, 'admin.feed_card.rotation_hint', 'admin', true, '랭킹 카드가 피드에서 순환되는 방식', 'How ranking cards cycle through positions', 'ランキングカードの表示方法', '排名卡在信息流中的循环方式', '排名卡在動態中的循環方式', 'Cómo se rotan las tarjetas de ranking', 'Comment les cartes de classement alternent', 'Wie Ranking-Karten rotieren', 'Como os cards de ranking são alternados', 'Cách thẻ xếp hạng luân phiên', 'วิธีหมุนเวียนการ์ดจัดอันดับ', 'Cara kartu peringkat berputar', 'كيفية تدوير بطاقات التصنيف', now(), now())
