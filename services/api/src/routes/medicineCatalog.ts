...
/**
 * Medicine Catalog — mirrors supplementCatalog.ts but filters by category_type='medicine'
 * and uses master_categories.code='medicine_category' for type hierarchy.
 * No nutrition endpoints — medicine metadata lives in feed_models.description JSON.
 */
export async function handleMedicineCatalog(request: Request, env: Env, url: URL): Promise<Response> {
  ...
  if (!isAdmin) {
    ...
    if (path === '/api/v1/medicine-catalog/types' && method === 'GET') {
      const rows = await env.DB.prepare(
        `SELECT
         ...
         LEFT JOIN i18n_translations tr
           ...
           END
         WHERE mc.code = 'medicine_category'
    ...
    if (path === '/api/v1/medicine-catalog/manufacturers' && method === 'GET') {
    ...
    if (path === '/api/v1/medicine-catalog/brands' && method === 'GET') {
    ...
    if (path === '/api/v1/medicine-catalog/models' && method === 'GET') {
  ...
  // ── Stats ────────────────────────────────────────────────────────────────
  if (path === '/api/v1/admin/medicine-catalog/stats' && method === 'GET') {
    const [totalRow, activeRow] = await Promise.all([
      env.DB.prepare(`SELECT COUNT(*) AS cnt FROM feed_models WHERE category_type = 'medicine'`).first<{ cnt: number }>(),
      env.DB.prepare(`SELECT COUNT(*) AS cnt FROM feed_models WHERE category_type = 'medicine' AND status = 'active'`).first<{ cnt: number }>(),
    ]);
    return ok({
  ...
  if (path === '/api/v1/admin/medicine-catalog/types' && method === 'GET') {
    const rows = await env.DB.prepare(
      `SELECT
         mi.id,
         mi.code AS key,
  ...
  if (path === '/api/v1/admin/medicine-catalog/manufacturers' && method === 'GET') {
  ...
  if (path === '/api/v1/admin/medicine-catalog/manufacturers' && method === 'POST') {
    ...
    const key = `med_mfr_${randomToken(8)}`;
    const nameKey = `medicine.manufacturer.${key}`;
    const id = newId();
    ...
    await upsertI18n(env, nameKey, normalizedTranslations(body.translations, ko, en), 'medicine');
    if (hasMfrTypeMap) {
  ...
  const mfrIdMatch = path.match(/^\/api\/v1\/admin\/medicine-catalog\/manufacturers\/([^/]+)$/);
  if (mfrIdMatch) {
    ...
    if (method === 'PUT') {
      ...
      if (existing.name_key && (ko || en)) {
        await upsertI18n(env, existing.name_key, normalizedTranslations(body.translations, ko, en), 'medicine');
      }
  ...
  if (path === '/api/v1/admin/medicine-catalog/brands' && method === 'GET') {
    const manufacturerId = url.searchParams.get('manufacturer_id');
  ...
  if (path === '/api/v1/admin/medicine-catalog/brands' && method === 'POST') {
    ...
    const key = `med_brand_${randomToken(8)}`;
    const nameKey = `medicine.brand.${key}`;
    const id = newId();
    await env.DB.prepare(
  ...
  const brandIdMatch = path.match(/^\/api\/v1\/admin\/medicine-catalog\/brands\/([^/]+)$/);
  if (brandIdMatch) {
  ...
  if (path === '/api/v1/admin/medicine-catalog/models' && method === 'GET') {
    const typeId = url.searchParams.get('feed_type_id');
    const mfrId = url.searchParams.get('manufacturer_id');
    const brandId = url.searchParams.get('brand_id');
    const hasModelBrandMap = (await hasTable(env, 'feed_model_brand_map'))
    ...
    try {
      ...
      console.error('[medicine-catalog admin models] primary query failed, fallback query used', e);
      const rows = await env.DB.prepare(
        `SELECT m.*,
  ...
  if (path === '/api/v1/admin/medicine-catalog/models' && method === 'POST') {
    ...
    const key = `med_model_${randomToken(8)}`;
    const nameKey = `medicine.model.${key}`;
    const id = newId();
    await env.DB.prepare(
  ...
  const modelIdMatch = path.match(/^\/api\/v1\/admin\/medicine-catalog\/models\/([^/]+)$/);
  if (modelIdMatch) {
    ...
    if (method === 'PUT') {
      ...
      if (existing.name_key && (ko || en || body.translations)) {
        await upsertI18n(env, existing.name_key, normalizedTranslations(body.translations, ko || existing.model_name || '', en || ko || existing.model_name || ''), 'medicine');
      }
      if (brandIds.length > 0 && await hasTable(env, 'feed_model_brand_map')) {
