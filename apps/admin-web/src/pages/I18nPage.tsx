...
export default function I18nPage() {
  ...
  async function handleDeactivate(id: string) {
    if (!confirm('이 번역 키를 비활성화하시겠습니까?')) return;
    try {
      await api.i18n.deactivate(id);
      setSuccess('비활성화되었습니다.'); void load();
      setTimeout(() => setSuccess(''), 3000);
    } catch (e) { setError(e instanceof Error ? e.message : 'Error'); }
  }
  ...
  return (
    <>
      <div className="topbar">
        <div className="topbar-title">🌐 언어 관리 (i18n)</div>
        <button className="btn btn-primary btn-sm" onClick={openCreate}>+ 번역 키 추가</button>
      </div>
        ...
        <div className="card">
          <div className="card-header">
            <div className="filters" style={{ margin: 0 }}>
              <input className="form-input" placeholder="키 접두사 검색 (예: app_, nav.)" value={prefix}
          ...
          <div className="table-wrap">
            {loading ? <div className="loading-center"><span className="spinner" /></div> : (
              <table>
                <thead>
                  <tr>
                    <th>키</th><th>페이지</th><th>한국어</th><th>English</th>
                    <th>상태</th><th>작업</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={row.id}>
                      ...
                      <td>
                        <div className="td-actions">
                          <button className="btn btn-secondary btn-sm" onClick={() => openEdit(row)}>편집</button>
                          {row.is_active ? (
                            <button className="btn btn-danger btn-sm" onClick={() => handleDeactivate(row.id)}>비활성화</button>
                          ) : null}
                        </div>
                      </td>
                    </tr>
                  ))}
                  {rows.length === 0 && <tr><td colSpan={6} style={{ textAlign: 'center', padding: 32, color: 'var(--text-muted)' }}>데이터가 없습니다</td></tr>}
                </tbody>
              </table>
            )}
          </div>
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && closeModal()}>
          <div className="modal">
            <div className="modal-header">
              <div className="modal-title">{modal === 'create' ? '번역 키 추가' : '번역 수정'}</div>
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-primary" onClick={handleSave} disabled={saving}>
                {saving ? <><span className="spinner" /> 저장중...</> : '저장'}
              </button>
            </div>
          </div>
        </div>
