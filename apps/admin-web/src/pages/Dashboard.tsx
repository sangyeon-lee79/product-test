  ...
  return (
    <>
      <div className="topbar">
        <div className="topbar-title">분석 대시보드</div>
      </div>
      <div className="content">
        <div className="form-row col3 mb-4">
          {[
            { label: 'API 상태', value: health?.status === 'ok' ? '정상' : health ? '오류' : '확인중...', color: health?.status === 'ok' ? '#38a169' : '#e53e3e' },
            { label: '환경', value: health?.environment || '-', color: '#1a73e8' },
            { label: 'DB', value: health?.services?.db || '연결중...', color: health?.services?.db === 'connected' ? '#38a169' : '#d69e2e' },
          ].map(stat => (
        ...
        <div className="card">
          <div className="card-header">
            <div className="card-title">📋 개발 진행 현황 (PLAN.md)</div>
          </div>
          <div className="card-body">
            {[
              ...
              { slice: 'S3 질병 연결 매핑', done: false, note: '다음 슬라이스' },
